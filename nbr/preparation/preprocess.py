...
class Preprocess:
    ...
    def load_data(self, user_min, item_min, filt=True):
        self._load_clicks(user_min, item_min)
        if filt:
            self._filter()
        self._assign_id()
        print('After preprocessing: #users = {}, #items = {}, #clicks = {}'.format(self.n_users, self.n_items, self.n_clicks))
        sys.stdout.flush()
    ...
    def _load_clicks(self, user_min, item_min):
        ...
        with open(self.dataset_path) as f:
            for n_read, line in enumerate(f):
                info = line.replace("\n", "").split('\t')
                try:
                    user_name, item_name, t = info[0].strip(), info[1].strip(), int(info[2])
                ...
                total_clicks += 1
        print("Before preprocessing: #users = {}, #items = {}, #clicks = {} (#illegal records = {})".format(
            len(user_cnt), len(item_cnt), total_clicks, illegal_records))
        sys.stdout.flush()
    ...
    def _filter(self):
        # filter noise users (who has high purchase frequency (> mean + 2 * std) or who boght all in one day)
        ...
        self.pos_per_user = np.array(self.pos_per_user, dtype="object")[indice]
    ...
    def _gen_valid_test(self):
        # test - last basket of user, validation - one before last basket of user (if user has < 3 baskets - all his history in train)
...
def save_split(corpus_path, dataset, corpus):
    ...
    print('Saving dataset in ' + corpus_path + '/data_{}/...'.format(dataset))
    sys.stdout.flush()
    # Book
