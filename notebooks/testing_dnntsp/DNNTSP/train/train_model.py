...
def train_model(model: nn.Module,
    ...
    for epoch in range(epochs):
        ...
        for name in name_list:
            if name == "validate" and validate_flg == False and epoch != epochs - 1:
                loss_dict[name] = loss_dict["train"]
                metric_dict[name] = metric_dict["train"]
                continue
            # training
            if name == "train":
                model.train()
            ...
            if name == "validate":
                ...
                if validate_ndcg > validate_max_ndcg:
                    ...
                    model_path = f"{model_folder}/best_model.pkl"
                    save_model(model, model_path)
                    print(f"model save as {model_path}")
                    best_score = metric_dict["validate"]["ndcg_10"]
